...
Should be rebound thread-locally since there is concurrency control is
not performed.")
...
(defun invoke-with-caches (bindings thunk)
  "Invoke THUNK with cache variable bound according to CACHES."
  (progv (mapcar #'first bindings) (mapcar #'second bindings)
    (funcall thunk)))

(defmacro with-caches (bindings &body body)
  "Execute BODY with cache variables bound according CACHES. BINDINGS
has either to be a list the elements of which are of the form

  (VAR INITIAL-VALUE-EXPR)

where VAR names the cache variable and INITIAL-VALUE-EXPR is evaluated
to produce the initial value of VAR or an expression which evaluates
to a list of described form. "
  (if (typep bindings '(or null (cons (cons symbol (cons t null)))))
      `(let (,@bindings) ,@body)
      `(invoke-with-caches ,bindings  (lambda () ,@body))))
